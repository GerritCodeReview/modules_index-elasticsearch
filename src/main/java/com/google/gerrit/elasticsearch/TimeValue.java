...
/** A modified version of org.elasticsearch.common.unit.TimeValue. */
public class TimeValue implements Comparable<TimeValue> {
  ...
  public TimeValue(long duration, TimeUnit timeUnit) {
    if (duration < -1) {
      throw new IllegalArgumentException(
          "duration cannot be negative, was given [" + duration + "]");
    }
   ...
   *
   * <p>Note that this method might produce fractional time values (ex 1.6m) which cannot be parsed
   * by method like {@link TimeValue#parse(String, String, String, String)}.
   *
   * <p>Also note that the maximum string value that will be generated is {@code 106751.9d} due to
   * the way that values are internally converted to nanoseconds (106751.9 days is Long.MAX_VALUE
   * nanoseconds)
   */
  @Override
   ...
   *
   * <p>Note that this method might produce fractional time values (ex 1.6m) which cannot be parsed
   * by method like {@link TimeValue#parse(String, String, String, String)}. The number of
  ...
  public static TimeValue parseTimeValue(
      String sValue, TimeValue defaultValue, String settingName) {
    Objects.requireNonNull(settingName);
    if (sValue == null) {
      return defaultValue;
    }
    ...
    if (normalized.endsWith("nanos")) {
      ...
      // parsing minutes should be case-sensitive as 'M' means "months", not "minutes"; this is the
      // only special case.
  ...
  private static long parse(
      final String initialInput, final String normalized, final String suffix, String settingName) {
